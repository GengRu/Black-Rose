<template>
	<div id="search">
		...
		<div class="elr-search">
			<el-input placeholder="请输入商家或美食名称" v-model="input" clearable class="elr-search-input"></el-input>
			<el-button type="primary" class="search-elr-search-btn" @click="searchBtn">搜索</el-button>
		</div>
		...
		<div class="elr-conts" v-show="type" v-if="losArr.length != 0">
			...
			<div class="elr-conts-cont" v-for="(i, $idx) in losArr" :key="$idx">
				{{i}}
				<span class="el-icon-close elr-conts-cont-removeBtn" @click="removeBtn($idx)"></span>
			</div>
...
<script>
	...
	export default {
		...
		methods:{
			searchBtn(){
				if(this.input != ''){
					this.axios.get('http://elm.cangdu.org/v4/restaurants?extras[]=restaurant_activity&geohash=31.22299,121.36025&keyword='+this.input+'&type=search').then(data=>{
						this.err = '很抱歉！无搜索结果'
						this.type = false
...
<style>
	...
	.elr-err{
		margin: 0 auto;
		font: .45rem/1.05rem Microsoft YaHei;
		color: #333;
