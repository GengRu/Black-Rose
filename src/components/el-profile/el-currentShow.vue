<template>
  <div>
    <section class="elt-content_container">
      <section class="elt-content">
        <header class="elt-content_header">
          <span class="elt-content_title_style">
            <slot name="nowHave"></slot>
          </span>
          <router-link :to="$route.path + $route.path + '_detail'">
            <section class="elt-contetn_description">
              <img
                class="elt-content_img"
                ...
                width="18"
              />
              <a href="#/balance/detail" class="elt-content_title">
                <slot name="explain"></slot>
              </a>
            </section>
          </router-link>
        </header>
        <p class="elt-content_num">
          <slot name="money"></slot>
          <slot name="unit"></slot>
        </p>
        <div class="elt-promit_button">
          <slot name="getSome"></slot>
        </div>
      </section>
    </section>
    <p class="elt-deal_detail">
      <slot name="log"></slot>
    </p>
    <div class="elt-no_log">
      <img
...
.elt-contetn_description {
  display: flex;
  align-items: center;
  font-size: 0.4rem;
}
.elt-content_num span:first-of-type {
  font-size: 1rem;
  color: #333;
  margin-right: 0.3rem;
}
.elt-content_num span:nth-of-type(2) {
  font-size: 0.5rem;
  color: #333;
}
.elt-promit_button {
...
.elt-no_log p:nth-of-type(2) div {
  font-size: 0.36rem;
  color: #999;
}
</style>
