<template>
 <div class="elw-children" ref="ch">
    <el_Header>
      ...
      <div slot="elw-cont">
        <span class="elg-title" v-for="(s,$s) in data" :key="$s">{{s.a}}</span>
      </div>
    </el_Header>
    <div class="elw-text" v-for="(i,$index) in data" :key="$index">
        <div v-if="arr" class="dss" ><img  :src="ae" alt="" width="100%" v-for="(ae,$ae) in arr" :key="$ae"></div>
        <section>{{i.b}}</section>
    </div>
  </div>
</template>
...
export default {
  ...
  watch:{
    'data'(s,n){
        for(let con in s){
            var str=s[con].b.substring(s[con].b.indexOf("(")+1,s[con].b.indexOf(")"))
            this.arr.push(str)
        }
    }
  }
