  ...
  <div id="pingjia" @scroll="scr($event)">
    <div class="elr-pingjia-cont">
      <div class="elr-pingjia-cont-l">
        <p class="elr-pingjia-cont-l-tt1">
          {{ scoreArr.overall_score.toFixed(1) }}
        </p>
        <p>综合评价</p>
        <p class="elr-pingjia-cont-l-tt2">
          高于周边商家的{{ (scoreArr.compare_rating * 100).toFixed(1) }}%
        </p>
      </div>
    ...
    <div class="elr-pingjia-cont2">
      <div class="elr-pingjia-conts">
        <span
          v-for="(i, $idx) in tagArr"
          :key="$idx"
          class="elr-pingjia-cont2-title"
          @click="num = $idx"
          :class="num == $idx ? 'elr-title-acitve' : ''"
      ...
      <div v-for="(i, $idx) in kehuArr" :key="$idx" class="elr-kehu">
          ...
          <img
            :src="
              i.avatar == ''
                ...
                : 'https://fuss10.elemecdn.com/' + i.avatar
            "
          />
        </div>
        ...
        <div class="elr-kehu-cont">
          ...
          <div class="elr-kehu-cont-xing-box">
            <el-rate
              v-model="i.rating_star"
              disabled
              show-score
              text-color="#ff9900"
              score-template=""
              class="elr-kehu-cont-xing"
            ></el-rate
            >{{ i.time_spent_desc }}
          </div>
          ...
          <div class="elr-kehu-bgc-cont">
            <div
              v-for="(j, $index) in i.item_ratings"
              :key="$index"
              v-show="j.image_hash != ''"
              class="elr-kehu-bgc"
            >
...
export default {
  ...
  created() {
    this.axios
      .get("http://elm.cangdu.org/ugc/v2/restaurants/1/ratings/scores")
      .then(data => {
    ...
    this.axios
      .get("http://elm.cangdu.org/ugc/v2/restaurants/1/ratings/tags")
      .then(data => {
        // console.log(data.data)
